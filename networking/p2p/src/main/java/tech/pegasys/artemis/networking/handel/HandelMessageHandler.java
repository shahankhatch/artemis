...
public class HandelMessageHandler implements Consumer<MessageApi> {
  private static final int MAX_SENT_MESSAGES = 2048;

  private static final Topic HANDEL_AGGREGATION_TOPIC = new Topic("/eth2/beacon_HandelAggregation/ssz");
  ...
  public HandelMessageHandler(final PubsubPublisherApi publisher, final EventBus eventBus) {
  ...
  public static void init(final Gossip gossip, final PrivKey privateKey, final EventBus eventBus) {
    ...
    final HandelMessageHandler handler = new HandelMessageHandler(publisher, eventBus);
    gossip.subscribe(handler, HANDEL_AGGREGATION_TOPIC);
